...
/**
 * JwtUtil unit tests for content-service.
 *
 * CRITICAL regression coverage: validateToken must return false (not throw)
 * for a token signed with a different key. In JJWT 0.12+ the signature
 * mismatch surfaces as io.jsonwebtoken.security.SignatureException, which
 * extends io.jsonwebtoken.security.SecurityException (NOT java.lang.SecurityException).
 * The previous `catch (SecurityException ex)` branch matched java.lang.SecurityException
 * and never caught the JJWT exception — resulting in 500 instead of 401.
 * This test class verifies the post-fix behaviour and locks the regression.
 *
...
class JwtUtilTest {
    ...
    class GenerateAndExtract {
        ...
        @Test
        @DisplayName("getUserIdFromToken returns null (content-service JwtUtil does not set userId claim)")
        ...
        @Test
        @DisplayName("getRoleFromToken returns null (content-service JwtUtil does not set role claim)")
    ...
    class Validate {
        ...
        @Test
        @DisplayName("REGRESSION: should return false (not 500/throw) for token signed with a different key")
        void shouldReturnFalseForDifferentSigningKey() {
            // Before the fix this threw io.jsonwebtoken.security.SignatureException which
            // was not caught (java.lang.SecurityException != JJWT SecurityException) and
            // bubbled out of validateToken → filter logged "Could not set user auth" and
            // returned 401 anyway only because the filter wraps validateToken in try/catch.
            // But any caller that did NOT wrap would receive 500. Lock the post-fix behaviour.
